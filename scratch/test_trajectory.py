    ...
    if latest_color is not None and latest_depth is not None:
        ...
        if time_diff < 30.0:
            ...
            if frame_count == 0:
                ...
                if track_success:
                    T_wc_raw = T_wc_prev @ T_rel
                    T_wc = T_wc_raw.copy()
                else:
                    T_wc_raw = T_pred.copy()
                    T_wc = T_pred.copy()
            ...
            if frame_count % 10 == 0:
                print(f"Frame {frame_count:03d} tracking: success={track_success}")
                print(f"  Raw T_wc Pos:   X={T_wc_raw[0, 3]:.4f}, Y={T_wc_raw[1, 3]:.4f}, Z={T_wc_raw[2, 3]:.4f}")
                print(f"  Filt T_wc Pos:  X={T_wc[0, 3]:.4f}, Y={T_wc[1, 3]:.4f}, Z={T_wc[2, 3]:.4f}")
                print(f"  ESKF Pos:       X={eskf.p[0]:.4f}, Y={eskf.p[1]:.4f}, Z={eskf.p[2]:.4f}")
                print(f"  ESKF Vel:       X={eskf.v[0]:.4f}, Y={eskf.v[1]:.4f}, Z={eskf.v[2]:.4f}")
                print(f"  ESKF Bias:      ba={eskf.ba}, bg={eskf.bg}")
