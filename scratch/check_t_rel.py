...
while reader.has_next() and frame_count < 30:
    ...
    if latest_color is not None and latest_depth is not None:
        ...
        if time_diff < 30.0:
            ...
            if frame_count == 0:
                ...
                print(f"Frame {frame_count:02d}: success={success}")
                print(f"  T_rel translation: X={T_rel[0,3]:.6f}, Y={T_rel[1,3]:.6f}, Z={T_rel[2,3]:.6f}")
                rpy = Rotation.from_matrix(T_rel[0:3, 0:3]).as_euler('xyz', degrees=True)
                print(f"  T_rel rotation RPY: {rpy}")
                T_wc = T_wc_prev @ T_rel
                print(f"  T_wc  translation: X={T_wc[0,3]:.6f}, Y={T_wc[1,3]:.6f}, Z={T_wc[2,3]:.6f}")
