...
print("Scanning bag for rejected frames...")
        ...
        if time_diff < 30.0:
            ...
            if frame_count > 0:
                ...
                if track_success:
                    ...
                    if dt_norm > 0.05:
                        is_rejected = True
                        reason.append(f"dt_norm={dt_norm:.4f}>0.05")
                    if gyro_mag > 0.4:
                        is_rejected = True
                        reason.append(f"gyro={gyro_mag:.2f}>0.4")
                        
                    if is_rejected:
                        print(f"Frame {frame_count:03d}: REJECTED due to {', '.join(reason)}. Raw T_rel translation = {np.linalg.norm(T_rel[0:3, 3])*100:.2f} cm")
                        
            latest_color_prev = latest_color.copy()
            depth_m_prev = depth_m.copy()
            T_wc_prev = T_pred.copy() # fallback style
            frame_count += 1
            latest_color = None
            latest_depth = None
