...
print("Starting calibrated tracking simulation...")
    ...
    if latest_color is not None and latest_depth is not None:
        ...
        if time_diff < 30.0:
            ...
            if frame_count == 0:
                ...
                if track_success:
                    # Sanity check: only reject absolute tracking failures
                ...
                if track_success:
                    ...
                    # Fallback (corrected coordinate transform)
                    eskf.v = eskf.v * 0.2
                    if eskf.is_gravity_initialized:
            ...
            if track_success:
                ...
                # DO NOT scale up pos_scale, keep it to 1.0 to let VO correct velocity drift!
                pos_scale = 1.0
            ...
            if frame_count % 30 == 0 or (frame_count >= 130 and frame_count <= 160 and frame_count % 5 == 0):
                p = T_wc[0:3, 3]
