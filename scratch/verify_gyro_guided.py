...
# We also apply the default offline_runner roll offset of -3.5 degrees
...
# Instantiate the GyroGuidedEstimator
...
print("Starting verification simulation with GyroGuidedEstimator...")
while reader.has_next() and frame_count < 450:
    ...
    if latest_color is not None and latest_depth is not None:
        ...
        if time_diff < 30.0:
            ...
            # Propagate GyroGuidedEstimator using IMU
            ...
            if frame_count == 0:
                ...
                # Damp predicted translation by 50%
                ...
                if track_success:
                    ...
                    if dt_norm > 0.15:
                        track_success = False
                        print(f"Frame {frame_count}: Tracking rejected (dt_norm = {dt_norm:.4f})")
            ...
            if track_success:
                T_wi = T_wc @ T_i2c
                # gyro_guided uses signature update(self, p_meas, R_meas, *args, **kwargs)
            ...
            if frame_count % 30 == 0 or (frame_count >= 130 and frame_count <= 160 and frame_count % 5 == 0) or (frame_count >= 370 and frame_count <= 400 and frame_count % 5 == 0):
                p = T_wc[0:3, 3]
                rpy = Rotation.from_matrix(T_wc[0:3, 0:3]).as_euler('xyz', degrees=True)
                print(f"F{frame_count:03d}: Pos = {p}, RPY = {rpy}, Gyro_mag = {np.linalg.norm(curr_gyr):.3f}")
