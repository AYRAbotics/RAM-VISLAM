...
def run_trajectory(pitch_offset=0.0):
    ...
    # Apply pitch offset
    R_corr = Rotation.from_euler('xyz', [0.0, np.radians(pitch_offset), 0.0]).as_matrix()
...
if __name__ == "__main__":
    for poff in [-4.0, -2.0, 0.0, 2.0, 4.0]:
        poses = run_trajectory(poff)
        poses = np.array(poses)
        print(f"Pitch Offset: {poff:+.1f} deg")
        print(f"  Start Pos (F0): X={poses[0,1]:.4f}, Y={poses[0,2]:.4f}, Z={poses[0,3]:.4f}")
        print(f"  End Pos (F100): X={poses[100,1]:.4f}, Y={poses[100,2]:.4f}, Z={poses[100,3]:.4f}")
        print(f"  End Pos (F200): X={poses[200,1]:.4f}, Y={poses[200,2]:.4f}, Z={poses[200,3]:.4f}")
        diff100 = poses[100, 1:4] - poses[0, 1:4]
        diff200 = poses[200, 1:4] - poses[0, 1:4]
        print(f"  F0->F100 delta: X={diff100[0]:.4f}, Y={diff100[1]:.4f}, Z={diff100[2]:.4f}")
        print(f"  F0->F200 delta: X={diff200[0]:.4f}, Y={diff200[1]:.4f}, Z={diff200[2]:.4f}")
